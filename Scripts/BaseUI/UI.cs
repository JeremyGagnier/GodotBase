...
namespace Base
{
	public class UI
	{
		...
		private readonly static Script BUTTON_SCRIPT = GD.Load<Script>("Scripts/BaseUI/NodeScripts/ButtonScript.cs");

		// Every UI component is recorded in elements and every node is recorded in elementNodes, in addition to their own
		// lists.
		[JsonInclude] public static ManagedList<ElementState> elements = new();
		public readonly static List<Node?> elementNodes = new();

		[JsonInclude] public static ManagedList<ButtonState> buttons = new();
		public readonly static List<Button?> buttonNodes = new();

		[JsonInclude] public static ManagedList<PanelState> panels = new();
		public readonly static List<Panel?> panelNodes = new();

		[JsonInclude] public static ManagedList<HBoxContainerState> hBoxContainers = new();
		public readonly static List<HBoxContainer?> hBoxContainerNodes = new();

		[JsonInclude] public static ManagedList<GridContainerState> gridContainers = new();
		public readonly static List<GridContainer?> gridContainerNodes = new();

		[JsonInclude] public static ManagedList<TextEditState> textEdits = new();
		...
		public static IdInfo AddButton(
			...
			Control.SizeFlags hSizeFlags = Control.SizeFlags.Fill,
			Control.SizeFlags vSizeFlags = Control.SizeFlags.Fill,
			bool disabled = false,
			bool visible = true)
		{
			...
			ButtonState buttonState = new()
			{
				...
				HSizeFlags = hSizeFlags,
				VSizeFlags = vSizeFlags,
			};
			int buttonId = buttons.Add(buttonState);
			MakeButton(buttonId, buttonState);
		...
		public static void ModifyButton(
			...
			Control.SizeFlags? hSizeFlags = null,
			Control.SizeFlags? vSizeFlags = null,
			bool? disabled = null,
			bool? visible = null)
		{
			...
			else
			{
				...
				if (hSizeFlags != null)
				{
					buttonState.HSizeFlags = hSizeFlags.Value;
				}
				if (vSizeFlags != null)
				{
					buttonState.VSizeFlags = vSizeFlags.Value;
				}
				if (visible != null)
				{
				...
				if (!Root.instance.headlessMode)
				{
					...
					if (button != null)
					{
						...
						if (hSizeFlags != null)
						{
							button.SizeFlagsHorizontal = hSizeFlags.Value;
						}
						if (vSizeFlags != null)
						{
							button.SizeFlagsVertical = vSizeFlags.Value;
						}
						if (disabled != null)
						{
		...
		public static void RemoveButton(int buttonId)
		{
			...
			if (buttonState != null)
			{
				...
				UnmakeButton(buttonId, buttonState);
			}
		}
				...
				if (elementState != null)
				{
					...
					if (parentNode != null)
					{
						Button button = new()
						{
							...
							SizeFlagsHorizontal = buttonState.HSizeFlags,
							SizeFlagsVertical = buttonState.VSizeFlags,
						};
						parentNode.AddChild(button);
						button.Pressed += UIActions.buttonActions[buttonState.OnPressActionName];
						ulong childId = button.GetInstanceId();
						button.SetScript(BUTTON_SCRIPT);
						Button? maybeButton = GodotObject.InstanceFromId(childId) as Button;
						if (maybeButton != null)
						{
							button = maybeButton;
						}
						((ButtonScript)button).nodeId = buttonState.ElementId;
		...
		public static void RemovePanel(int panelId)
		{
			...
			if (panelState != null)
			{
				...
				UnmakePanel(panelId, panelState);
			}
		}
			...
			bool visible = true)
		{
			...
			MakeHBoxContainer(hBoxContainerId, hBoxContainerState);
		...
		public static void RemoveHBoxContainer(int hBoxContainerId)
		{
			...
			if (hBoxContainerState != null)
			{
				...
				UnmakeHBoxContainer(hBoxContainerId, hBoxContainerState);
			}
		}
			...
			bool visible = true)
		{
			...
			MakeGridContainer(gridContainerId, gridContainerState);
		...
		public static void RemoveGridContainer(int gridContainerId)
		{
			...
			if (gridContainerState != null)
			{
				...
				UnmakeGridContainer(gridContainerId, gridContainerState);
			}
		}
			...
			bool visible = true)
		{
			...
			MakeTextEdit(textEditId, textEditState);
		...
		public static void RemoveTextEdit(int textEditId)
		{
			...
			if (textEditState != null)
			{
				...
				UnmakeTextEdit(textEditId, textEditState);
			}
		}
